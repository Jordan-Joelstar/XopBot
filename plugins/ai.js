...
const { TelegraPh, aitts, Index, Config, parsedJid, sleep, Draw, getDateTime } = require("../lib");
const { aiResponse } = require("../lib/ai");
Index(
 {
  pattern: "chat",
...
Index(
 ...
 async (message, text, { smd: cmd }) => {
  try {
   ...
   if (!input) {
    return await message.send('_Need Your Number, Message_');
   }
 ...
 async (message) => {
  try {
   ...
   if (!quotedMessage || !validTypes.includes(quotedMessage?.mtype)) {
    return await message.send("*_Reply to image_*");
   }
