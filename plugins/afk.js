...
bot({ pattern: "afk", fromMe: true, desc: Lang.AFK_DESC }, async (message, match) => {
 if (match[1] === "disable" && (message.fromMe || message.sender.split("@")[0] === global.sudo?.split(",")[0])) {
  AFK_DB.lastseen = 0;
  AFK_DB.reason = false;
  AFK_DB.isAfk = false;
  return await message.reply(Lang.IM_NOT_AFK);
