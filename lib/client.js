...
let { default: SuhailMDConnect, proto, prepareWAMessageMedia, downloadContentFromMessage, DisconnectReason, useMultiFileAuthState, generateForwardMessageContent, generateWAMessageFromContent, makeInMemoryStore, jidDecode } = require("@whiskeysockets/baileys");
var last_status = {};
global.setCmdAlias = {};
global.AstroOfficial = false;
global.sqldb = false;
global.pg_pools = false;
const { userdb, groupdb, bot_, smdBuffer } = require("./index.js");
...
async function syncdb() {
 ...
 sock.ev.on("connection.update", async (_0x3fd0b3) => {
  ...
  if (_0x2224a7 === "open") {
   if (/true|ok|sure|yes/gi.test(global.flush) || !sock.authState.creds?.myAppStateKeyId) {
    log("Flushing SESSION_ID" + (sock.authState.creds?.myAppStateKeyId ? "" : " B'Coz *myAppStateKeyId Missing") + "!");
    sock.ev.flush();
   }
        ...
        for (const fileName in sessionData) {
          try {
            fs.writeFileSync(
              ...
              typeof sessionData[fileName] == "string" ? sessionData[fileName] : JSON.stringify(sessionData[fileName], null, 2)
            );
          } catch (writeErr) {}
        }
      } else {
