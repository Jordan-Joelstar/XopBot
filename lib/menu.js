...
const { runtime, formatp, tiny, prefix, tlang } = require('.')
...
「 *${tiny(category)}* 」\n`
  ...
  if (/1|buttons|btn/gi.test(BUTTONS) && message.device !== 'web') {
   await sendButtons(message, {
    ...
    caption: menuText,
    buttons: `
 #button:quick_reply | display_text : Server| id: ${prefix}cpu /# 
 #button:quick_reply| display_text : Script | id:${prefix}repo /#            
